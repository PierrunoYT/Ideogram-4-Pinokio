...
// Sync the editor JSON into the component value so Python reads it as a normal input.
// Assigning props.value syncs back to gradio; priming lastPushed first stops the value
// watcher above from treating it as an external change (no re-render loop).
function syncValue(){
  const v = clean();
  lastPushed = JSON.stringify(v ?? null);
  props.value = v;
}
element.addEventListener("input", () => syncValue());
element.addEventListener("click", () => syncValue());
window.addEventListener("pointerup", () => syncValue());
